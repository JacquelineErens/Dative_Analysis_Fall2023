...
#cosine was significant in the negative direction when included alone
#Now lets try adding in length measures

PO_Recipient_Cosine_People <- brm(PO_Recipient_Duration ~ scaled_cosine_people *
                                   scaled_num_syllables_second_person *
                                   scaled_num_syllables_item + 
...
#model with PMI of people and items, as well as syllable length (mirroring decisions made for MSc thesis?)
PO_Recipient_Complex1 <- brm(PO_Recipient_Duration ~ 
                                   scaled_pmi_people *
                                   scaled_pmi_items *
                                   scaled_num_syllables_second_person *
                                   scaled_num_syllables_item +
                     ...
                     iter = 20000, thin = 10, save_pars = save_pars(all = TRUE),
                     data = PO_data)
summary(PO_Recipient_Complex1)
plot(PO_Recipient_Complex1)

#but also for the recipient maybe only the person matters?
#although the item is in between so that should facilitate it?
PO_Recipient_Complex0 <- brm(PO_Recipient_Duration ~ 
                                   scaled_pmi_people *
                                   scaled_pmi_items *
                                   scaled_num_syllables_second_person +
                                   (1|ParticipantNum) + (1|Anchor),
                     ...
                     iter = 20000, thin = 10, save_pars = save_pars(all = TRUE),
                     data = PO_data)
summary(PO_Recipient_Complex0)
plot(PO_Recipient_Complex0)
```
...
PO_Recipient_scaled_name_agreement_second_person <- brm(PO_Recipient_Duration ~ 
                                                            scaled_name_agreement_second_person + (1|ParticipantNum),
                     control=list(adapt_delta=0.995, max_treedepth=15),
                     iter = 20000, thin = 10, save_pars = save_pars(all = TRUE),
                     data = PO_data)
